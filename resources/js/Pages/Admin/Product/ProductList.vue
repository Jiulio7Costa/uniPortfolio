...
const handleRemove = (file) => {
    ...
    if (index !== -1) productImages.value.splice(index, 1);
}
...
// Handle Close method for dialog close event
...
const openAddModal = () => {
    ...
    resetFormData(); // Reset the form when opening the Add Product modal
}
...
const openEditModal = (product) => {
    ...
    // Handle product images (already uploaded)
    product_images.value = product.product_images || [];

    //  Reset new upload queue
    productImages.value = [];
};
        ...
        await router.post('products/update/' + id.value, formData, {
            onSuccess: (page) => {
                ...
                window.swal.fire({
                    ...
                    title: page.props.flash.success,
                    position: 'top-end',
                    showConfirmButton: true,   // Shows "OK" button
...
<template>
    ...
    <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
        <!-- dialogue for adding product or to edit-->
                            ...
                            <tr v-for="product in products" :key="product.id" class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">
                                    <span v-if="product.inStock == 0"
                                        class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-green-900 dark:text-green-300">In
                                        Stock</span>
                                    <span v-else
